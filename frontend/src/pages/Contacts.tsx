...
function Contacts() {
    ...
    const handleSave = async () => {
      ...
      try {
        const response = await axios.put(`http://localhost:3000/contacts/${editingContact.email}`, editingContact);
        if (response.status === 200) {
          setContacts(contacts.map(contact => contact.email === editingContact.email ? response.data : contact));
  ...
  return (
    <div className="bg-[url('src/assets/contacts.png')] bg-cover bg-center flex flex-col min-h-screen justify-center items-center">
                  ...
                  <tr key={contact.email}>
                    ...
                    <td className="py-2 px-6">
                        {editingContact?.email === contact.email ? (
                          <input type="text" value={editingContact.full_name} onChange={(e) => handleChange(e, 'full_name')} />
                        ) : (
                          contact.full_name
                        )}
                      </td>
                      <td className="py-2 px-6">
                        {editingContact?.email === contact.email ? (
                          <input type="text" value={editingContact.gender} onChange={(e) => handleChange(e, 'gender')} />
                        ) : (
                          contact.gender
                        )}
                      </td>
                      <td className="py-2 px-6">
                        {editingContact?.email === contact.email ? (
                          <input type="email" value={editingContact.email} onChange={(e) => handleChange(e, 'email')} />
                        ) : (
                          contact.email
                        )}
                      </td>
                      <td className="py-2 px-6">
                        {editingContact?.email === contact.email ? (
                          <input type="text" value={editingContact.phone} onChange={(e) => handleChange(e, 'phone')} />
                        ) : (
                          contact.phone
                        )}
                      </td>
                      <td className="py-2 px-6">
                        <div className="flex justify-center">
                          {editingContact?.email === contact.email ? (
                            <>
                              <button onClick={handleSave} className="bg-[#093f47] hover:bg-teal-900 text-white  py-2 px-6 rounded-3xl border-2 border-[#093f47] mr-4 ">save</button>
                              <button onClick={handleCancel} className="bg-red-600 hover:bg-red-400 text-white  py-2 px-6 rounded-3xl border-2 border-[#093f47] ">cancel</button>
                            </>
                          ) : (
                            <>
      ...
      <Modal
        ...
        toggleModal={() => setIsSaveModalOpen(false)}
        title="Your contact has been saved successfully!"
      >
        <button onClick={() => setIsSaveModalOpen(false)} className="bg-[#093f47] hover:bg-teal-900 text-white  py-2 px-6 rounded-3xl border-2 border-[#093f47] mr-4 text-xl">OK</button>
      </Modal>
      </div>
